import {getModels, Model, service} from '../src'
...
describe('model function and props test', function () {
    it('test model class function', () => {
        ...
        model.add(10);
        model = <TestModel> getModels()[modelName];
    ...
    it('test model data update', () => {
        ...
        expect(model.num).toBe(0);
        model.setNum(1);
        expect(getModels()[modelName]).not.toEqual(model);
        model = <TestModel>getModels()[modelName];
        model.setNum(1);
        expect(getModels()[modelName]).toEqual(model);
        model = <TestModel>getModels()[modelName];
        model.setData({num: 1});
        expect(getModels()[modelName]).toEqual(model);
        model = <TestModel>getModels()[modelName];
